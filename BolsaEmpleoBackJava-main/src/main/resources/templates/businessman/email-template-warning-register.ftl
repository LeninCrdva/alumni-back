<html xmlns="http://www.w3.org/1999/xhtml" lang="es">

<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <title>Java Techie Mail</title>
    <style>

        .space-icon img {
            padding: 15px;
        }

        .space-icon a {
            text-decoration: none;
            color: black;
        }

    </style>
</head>

<body>
<table>
    <tr>
        <td style="background-color: white;"><br> <br>
            <table>
                <tr>
                    <td
                            style="background-color: white; font-family: Arial, Helvetica, sans-serif; font-size: 13px; color: #000000; padding: 0px 15px 10px 15px;">

                        <body>
                        <div class="center-icon">
                            <img src='https://i.postimg.cc/vDc408Nf/checked-1.png' alt='checked-icon' width="100px"/>
                        </div>
                        <div class="center-text">
                            <h2><strong>¡Un nuevo registro!</strong></h2>
                            <br>
                            <p>Estimado administrador/a: </p>
                            <br>
                            <p>Se ha detectado el registro de una nueva empresa, los datos se han guardado, pero, se
                                necesita supervisión y activación de la cuenta del empresario <strong>${fullName}</strong>,
                                representante de la empresa <strong>${empresa}</strong>.</p>
                            </p>
                            <br>
                            <p>Según la decisión tomada, se informará al representante del estado de su cuenta.</p>
                            <br>
                            <p>¡Gracias por confiar en nosotros!</p>
                            <p>El equipo de la ISTA.</p>
                            <br>
                        </div>
                        <div class="center-icon">
                            <img src="https://i.postimg.cc/7G2S7n52/logo-ista.png" alt="logo-ista" width="1000px">
                        </div>
                        <br>
                        <hr>
                        <div class="center-icon space-icon">
                            <a href="https://www.facebook.com/TecAzuay"><img
                                        src="https://upload.wikimedia.org/wikipedia/commons/thumb/b/b8/2021_Facebook_icon.svg/2048px-2021_Facebook_icon.svg.png"
                                        alt="face-ico" width="30px"></a>
                            <a href="https://www.instagram.com/tecdelazuay/?hl=es-la"><img
                                        src="https://upload.wikimedia.org/wikipedia/commons/thumb/5/58/Instagram-Icon.png/1200px-Instagram-Icon.png"
                                        alt="insta-ico" width="30px"></a>
                        </div>
                        </body>
                        <footer>
                            <small class="center-text">
                                <p>Copyright (C) 2024 <strong>Instituto Tecnológico del Azuay</strong>. All rights
                                    reserved.</p>
                                <p>Octavio Chacón Moscoso 1-98 y Primera Transversal (Parque industrial)</p>
                                <p>Cuenca, Ecuador</p><br>
                                <p>Teléfono: 07 2809 551 - 0995363076</p>
                                <p>O contáctanos a nuestro correo: info.alumni.est@gmail.com</p>
                            </small>
                        </footer>
                        </div>
                    </td>
                </tr>
            </table>
            <br> <br>
        </td>
    </tr>
</table>
</body>

</html>